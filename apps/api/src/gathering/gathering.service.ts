...
export class GatheringService {
  ...
  async writeCode(userId: string) {
    return this.infrastructure.transaction(async (client) => {
      ...
      const studio = await client.query<{ cash_bonus: string; fans_bonus: string }>('SELECT COALESCE(SUM(u.bonus_cash), 0) AS cash_bonus, COALESCE(SUM(u.bonus_fans), 0) AS fans_bonus FROM player_studio_upgrades p JOIN studio_upgrades u ON u.id = p.upgrade_id WHERE p.user_id = $1', [userId]);
      const cash = reward.cash + Number(studio.rows[0].cash_bonus); const fans = reward.fans + Number(studio.rows[0].fans_bonus);
      ...
      await client.query('UPDATE player_currencies SET cash = cash + $2, fans = fans + $3, updated_at = NOW() WHERE user_id = $1', [userId, cash, fans]);
      await client.query('INSERT INTO gathering_log (user_id, action, item_id, quantity, cash_awarded, fans_awarded) VALUES ($1, $2, $3, $4, $5, $6)', [userId, 'write-code', item.id, reward.quantity, cash, fans]);
      return { item: this.item(item), quantity: reward.quantity, cash, fans, nextAvailableAt: new Date(action.rows[0].last_write_code_at.getTime() + WRITE_CODE_COOLDOWN_SECONDS * 1000).toISOString() };
    });
  }
}
